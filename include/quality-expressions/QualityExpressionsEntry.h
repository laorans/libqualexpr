...
public:		// Access API
  QualityExpressionID_T		id(void) const							{ return m_id; }
  QualityExpression &		expression(void)						{ return m_expression; }
  const QualityExpression &	expression(void)  const						{ return m_expression; }

  void				setExpression(const QualityExpression &expression)		{ m_expression = expression; }
