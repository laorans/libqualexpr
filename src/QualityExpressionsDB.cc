 ...
 */
void QualityExpressionDB::registerQualityExpression(const QualityExpression &expression, std::vector<QualityExpressionID_T> &p_idList)
{
  ...
  do {
    if (nextCur != std::string::npos) subExpression = expression.substr(cur, nextCur-cur);
    else subExpression = expression.substr(cur);
...
void QualityExpressionDB::registerQualityExpressionEntry(const QualityExpression &expression, std::vector<QualityExpressionID_T> &p_idList)
{
...
const QualityExpression & QualityExpressionDB::getQualityExpressionById(QualityExpressionID_T id) throw(QualityExpressionDB::Exception)
{
  ...
  if (dbItem == m_qualityExpressionsByID.end()) throw(Exception("QualityExpressionDB::getQualityExpressionById: not found"));
  return dbItem->second->expression();
}

void QualityExpressionDB::display(const std::string &indent, std::stringstream &s) const
{
  ...
  for (QualityExpressionSet_T::const_iterator dbItem = m_qualityExpressionsSet.begin(); dbItem != m_qualityExpressionsSet.end(); dbItem++ ) {
    s<<indent; dbItem->second->display(nindent, s);
  }
}
