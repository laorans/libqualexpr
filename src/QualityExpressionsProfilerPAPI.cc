...
*/
  ...
  private:	// -- PAPI util functions.
    static void				processError(const std::string &message, int retVal) throw (QualExprProfilerPapi::Exception);
  ...
  /* Constructor */ QualExprProfilerPapi::QualExprProfilerPapi(void) : QualExprSemaphore(), m_papiEventSet(PAPI_NULL), m_eidCursor(0), m_listenerList(), m_semanticNamespace(NULL)
  {
    ...
    if (initDone == PAPI_NOT_INITED) {
      ...
      if (err != PAPI_VER_CURRENT) processError("PAPI library version mismatch", err);
      err = PAPI_thread_init(pthread_self);
      if (err != PAPI_OK) processError("PAPI thread initialization error", err);
    }

    int err = PAPI_create_eventset(&m_papiEventSet);
    if (err != PAPI_OK) processError("Creating event sets", err);

    const PAPI_hw_info_t *hw_info = PAPI_get_hardware_info();
    if (hw_info) memcpy((void *) &m_papi_hardware_info, (const void *) hw_info, sizeof(m_papi_hardware_info));
    else processError("Could not get papi hardware info", err);
  }

  /* Destructor */QualExprProfilerPapi::~QualExprProfilerPapi(void)
  {
    if (m_semanticNamespace) delete m_semanticNamespace;
    m_semanticNamespace = NULL;
  }
  ...
  const QualExprSemanticNamespace &QualExprProfilerPapi::registerListener(listen_event_func_t listener)
  {
    ...
    if (!m_semanticNamespace) m_semanticNamespace = new QualExprSemanticNamespacePAPI;
   ...
   */
  void QualExprProfilerPapi::processError(const std::string &message, int retVal) throw (QualExprProfilerPapi::Exception)
  {
    std::stringstream s;
...
#undef QUALEXPRSEMANTIC_PAPI_DEF
    ...
    if (semantic <= QE_PROFILER_PAPI_PRESET_BASE || semantic >= QE_PROFILER_PAPI_NOMORE) throw (Exception("Illegal papi event"));
...
#undef QUALEXPRSEMANTIC_PAPI_DEF
    ...
    if (semantic <= QE_PROFILER_PAPI_BASE || semantic >= QE_PROFILER_PAPI_NOMORE) throw (Exception("Illegal papi event"));
  ...
  QualExprProfilerPapi::long64_papi_t QualExprProfilerPapi::getInfo(enum qualexpr_event_papi_t semantic) throw (QualExprProfilerPapi::Exception)
  {
    ...
    switch(semantic) {
    case QE_PROFILER_PAPI_INF_CORES         : result = m_papi_hardware_info.cores	        ; /* */ break;
    case QE_PROFILER_PAPI_INF_CPU_MAX_MHZ   : result = m_papi_hardware_info.cpu_max_mhz 	; /* */ break;
    case QE_PROFILER_PAPI_INF_CPU_MIN_MHZ   : result = m_papi_hardware_info.cpu_min_mhz   	; /* */ break;
    case QE_PROFILER_PAPI_INF_CPUID_FAMILY  : result = m_papi_hardware_info.cpuid_family  	; /* */ break;
    case QE_PROFILER_PAPI_INF_CPUID_MODEL   : result = m_papi_hardware_info.cpuid_model   	; /* */ break;
    case QE_PROFILER_PAPI_INF_CPUID_STEPPING: result = m_papi_hardware_info.cpuid_stepping	; /* */ break;
    case QE_PROFILER_PAPI_INF_CPU_MODEL     : result = m_papi_hardware_info.model	     	; /* */ break;
    case QE_PROFILER_PAPI_INF_NCPU          : result = m_papi_hardware_info.ncpu          	; /* */ break;
    case QE_PROFILER_PAPI_INF_CPU_NNODES    : result = m_papi_hardware_info.nnodes  	  	; /* */ break;
    case QE_PROFILER_PAPI_INF_CPU_REVISION  : result = m_papi_hardware_info.revision  		; /* */ break;
    case QE_PROFILER_PAPI_INF_CPU_SOCKETS   : result = m_papi_hardware_info.sockets	   	; /* */ break;
    case QE_PROFILER_PAPI_INF_CPU_THREADS   : result = m_papi_hardware_info.threads   		; /* */ break;
    case QE_PROFILER_PAPI_INF_TOTALCPUS     : result = m_papi_hardware_info.totalcpus     	; /* */ break;
    case QE_PROFILER_PAPI_INF_VENDOR        : result = m_papi_hardware_info.vendor        	; /* */ break;
  ...
  bool QualExprProfilerPapi::isAvailable(enum qualexpr_event_papi_t semantic)
  {
    ...
    if (kind == PAPI_KIND_INFO) isAvailable = true;
      ...
      if (err != PAPI_OK) {
        ...
        std::stringstream s; s << "Failed to get info on PAPI counter " << eventName;
        processError(s.str(), err);
      }
      else isAvailable = (info.count > 0);
    }
    return isAvailable;
  }
  ...
  void QualExprProfilerPapi::addSemantic(enum qualexpr_event_papi_t semantic) throw(QualExprProfilerPapi::Exception)
  {
    ...
    if (kind == PAPI_KIND_INFO) {
      ...
      if (!found) m_infoList.push_back(semantic);
      unlock();
    }
    else if (kind == PAPI_KIND_PRESET) {
      int err = PAPI_OK;
      ...
      if (err != PAPI_OK) {
        char eventName[PAPI_MAX_STR_LEN];
        PAPI_event_code_to_name(papi_counter, eventName);
        std::stringstream s; s << "Failed adding PAPI counter " << eventName;
        processError(s.str(), err);
      }
    }
    else throw (Exception("Illegal papi event"));
  }
  ...
  void QualExprProfilerPapi::reset(void) throw(QualExprProfilerPapi::Exception)
  {
    ...
    unlock();
    if (err != PAPI_OK) processError("Failed removing PAPI counter", err);
  }
  ...
  void QualExprProfilerPapi::startCounters(void)
  {
    ...
    *currentEvent = event;
    ...
    if (!m_counterList.empty()) {
      ...
      unlock();
      if (err != PAPI_OK) processError("Failed starting PAPI counter", err);
    }
  }
...
#else
  static QualExprSemanticNamespacePAPI * g_semanticNamespace = NULL;		//!< The quality expression namespace built for the PAPI profiler.
  static void qualExpr_papi_nopapierror(void)
  {
    static size_t g_errorMessageSent = 0;
    if (!g_errorMessageSent) fprintf(stderr, "Internal papi error: PAPI counters disabled\n");
    g_errorMessageSent++;
  }
  ...
  extern "C" {
    semantic_namespace_t qualExpr_papi_registerListener(listen_event_func_t listener)
    {
      if (!g_semanticNamespace) g_semanticNamespace = new QualExprSemanticNamespacePAPI;
      return (semantic_namespace_t) g_semanticNamespace;
    }

    void qualExpr_papi_unregisterListener(listen_event_func_t listener)				{ qualExpr_papi_nopapierror(); }
    void qualExpr_papi_startCounters(void)							{ qualExpr_papi_nopapierror(); }
    void qualExpr_papi_stopCounters(void)							{ qualExpr_papi_nopapierror(); }
    enum qualexpr_kind_papi_t qualExpr_papi_eventKind(enum qualexpr_event_papi_t semantic)	{ qualExpr_papi_nopapierror(); return PAPI_KIND_UNDEF; }
    int qualExpr_papi_isAvailable(enum qualexpr_event_papi_t semantic)				{ return false; }
    void qualExpr_papi_addSemantic(enum qualexpr_event_papi_t semantic)				{ qualExpr_papi_nopapierror(); }
    void qualExpr_papi_reset(void)								{ qualExpr_papi_nopapierror(); }
  }
#endif
